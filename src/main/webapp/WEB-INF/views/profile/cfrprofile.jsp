...
<body>
<!-- 프로필 이미지 불러오는 영역 -->
 <%String image = request.getParameter("image"); %> <!-- param image를 < %=image%> 변수에 저장  -->
...
window.onload = function(){
 ...
 for(int i=0 ; i<faces.length();i++ ){
	 ...
	 econfi2 = Math.round(econfi.doubleValue()*100) ;
		...
		// out.println("성별=" + gvalue + "나이=" + avalue + "감정=" + evalue+ "포즈=" +pvalue );
		// out.println("성별=" + gvalue + "\t (정확도="+ gconfi2 +"%)<br>");
		// out.println("나이=" + avalue4 + "\t (정확도="+ aconfi2 +"%)<br>");
		// out.println("감정=" + evalue + "\t (정확도="+ econfi2 +"%)<br>");
 }
 %>
	Gender: <%=gvalue  %> (<%=gconfi2 %> %)<br>
	Age: <%=avalue4 %> (<%=aconfi2 %> %)<br>
	Emotion: <%=evalue %> (<%=econfi2 %>)%
                </div>
                
 <form action=/selectMusic >
 	<input type='hidden' name="image_age" value='<%=(int)avalue4 %>'>
 	<input type='hidden' name="image_gender" value="<%=gvalue%>">
 	<input type='hidden' name="image_emotion" value="<%=evalue%>">
<!--  	<input type=submit value="추천음악듣기"> -->
 	<button class="btn btn-primary btn-xl text-uppercase"  type="submit" >음악 추천</button> 
